...
public class LogAnalyticService {
	...
	public Map<LocalDate, Long> groupingByDates(){
		return streamLogs().collect(groupingBy(LogEntry::getDate,counting()));
	}
	
	@RequestMapping("/grouping/datesThenHost")
	public Map<LocalDate, Map<String, Long>> groupingByDatesThenHosts(){
		return streamLogs().collect(groupingBy(LogEntry::getDate, groupingBy(LogEntry::getHost, counting())));
	}
	
	@RequestMapping("/grouping/datesThenResponse")
	public Map<LocalDate, Map<Integer, Long>> groupingByDatesThenResponse(){
		return streamLogs().collect(groupingBy(LogEntry::getDate, groupingBy(LogEntry::getResponse, counting())));
	}
	...
	private Stream<LogEntry> streamLogs() {
		try {
			return Files.list(Paths.get(loggingDir)).flatMap(this::lines).map(LogEntry::parse);
